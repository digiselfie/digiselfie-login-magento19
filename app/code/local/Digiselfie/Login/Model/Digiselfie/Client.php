...
class Digiselfie_Login_Model_Digiselfie_Client

{
    const XML_REDIRECT_URI_ROUTE = 'digiselfie/digiselfie_login/redirect_uri_route';
    const XML_PATH_ENABLED = 'digiselfie/digiselfie_login/enabled';
    const XML_PATH_CLIENT_ID = 'digiselfie/digiselfie_login/oauth_client_id';
    const XML_PATH_CLIENT_SECRET = 'digiselfie/digiselfie_login/oauth_secret';
    ...
    public function __construct($params = array())
    {
        if(($this->isEnabled = $this->_isEnabled())) {
            ...
            $this->redirectUri = Mage::getModel('core/url')->sessionUrlVar($this->_getRedirectUriRoute());
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('digiselfie_login')
                        ->__('Unspecified OAuth error occurred.');
                }
                throw new Digiselfie_Login_OAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('digiselfie_login')
...
class Digiselfie_Login_OAuthException extends Exception {}
